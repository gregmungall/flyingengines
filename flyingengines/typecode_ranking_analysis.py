...
with pg2.connect(
    ...
    with conn.cursor() as cur, open('typecode_ranking.sql', 'r') as sql:
        cur.execute(sql.read())
        data_raw = cur.fetchall()

# Create dataframe from raw data.
data_df = pd.DataFrame(
    data_raw, columns=['Date', 'Typecode', 'Daily flying hours', 'Ranking'])
data_df.to_csv('../csvs/typecode_ranking_data.csv', encoding='utf-8')
print(data_df)
...
plt.title(
    'Flying hours for top 3 aircraft per day between 2019-12-01 and 2020-11-30')
plt.tight_layout()
plt.savefig("../plots/typecode_ranking_plot.png")
plt.show()
